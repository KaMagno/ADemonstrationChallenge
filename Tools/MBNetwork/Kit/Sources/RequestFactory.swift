...
public struct RequestFactory {
    ...
    private func urlPath(from endpoint: some Endpoint) -> String {
        var urlPath = urlPath(from: endpoint.baseEndpoint)
    ...
    private func urlPath(from baseEndpoint: (any BaseEndpoint)?) -> String {
        guard let baseEndpoint = baseEndpoint else {
            return ""
        }
        
        var urlPath = urlPath(from: baseEndpoint.baseEndpoint)
